<template>
  <div class="min-h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-teal-50 pb-20">
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showEditModal && editingChore" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click="showEditModal = false">
          <div class="bg-white rounded-3xl max-w-md w-full shadow-2xl max-h-[90vh] overflow-y-auto" @click.stop>
            <div class="p-6 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-t-3xl sticky top-0 z-10">
              <h2 class="text-xl font-bold">Aufgabe bearbeiten</h2>
            </div>

            <form @submit.prevent="submitEditChore" class="p-6 space-y-4">
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Titel *</label>
                <input
                  v-model="editChoreData.title"
                  type="text"
                  required
                  class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none transition-colors"
                />
              ...
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Beschreibung</label>
                <textarea
                  v-model="editChoreData.description"
                  rows="3"
                  class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none transition-colors resize-none"
                ></textarea>
              </div>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-semibold text-gray-700 mb-2">Wiederholung *</label>
                  <select
                    v-model="editChoreData.recurrence_type"
                    required
                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none transition-colors"
                  >
                    <option value="daily">Täglich</option>
                ...
                <div>
                  <label class="block text-sm font-semibold text-gray-700 mb-2">Schwierigkeit *</label>
                  <select
                    v-model.number="editChoreData.difficulty_points"
                    required
                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none transition-colors"
                  >
                    <option :value="1">⭐ Leicht</option>
              ...
              <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Zuweisung *</label>
                <select
                  v-model="editChoreData.assignment_mode"
                  required
                  class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:outline-none transition-colors"
                >
                  <option value="manual">Manuell zuweisen</option>
              ...
              <div class="flex items-center gap-3 p-4 bg-blue-50 rounded-xl">
                <input
                  v-model="editChoreData.requires_photo"
                  type="checkbox"
                  id="edit_requires_photo"
                  class="w-5 h-5 text-blue-500 rounded"
                />
                <label for="edit_requires_photo" class="text-sm font-medium text-gray-700 cursor-pointer">
                  Foto-Nachweis erforderlich 📷
                </label>
              </div>
...
function handleEditChore(chore: any) {
  ...
  editChoreData.value = {
    ...
    difficulty_points: chore.difficulty_points,
...
async function submitEditChore() {
  ...
  try {
    await choreStore.updateChore(editingChore.value.id, editChoreData.value)
